    ...
    if (isLocalFile) {
      ...
      if (iosInstructions) {
        ...
        if (firstStep) {
          ...
          strong.textContent = '⚠️ Server Required:';

          const text1 = document.createTextNode(' PWAs must be served from a web server. Upload the ');

          const code = document.createElement('code');
          code.textContent = 'dist/';

          const text2 = document.createTextNode(' folder to free hosting (see options below) or use a local server on your computer.');
          ...
          firstStep.appendChild(stepNumber);
