 ...
 */
/**
 * Convert markdown to email-friendly plain text
 * Preserves structure while making it readable in email clients
 */
function formatReportForEmail(reportText: string): string {
  ...
  // Convert bold markdown to plain text emphasis
  formatted = formatted.replace(/\*\*(.+?)\*\*/g, '$1');
  formatted = formatted.replace(/\*(.+?)\*/g, '$1');
  
  // Convert list items to plain text with indentation
...
export function generateEmailReport(
  ...
  if (reportText) {
    // Format the report text for email
