...
interface AIResponseBubbleProps {
  ...
  onMoodChange?: (emotion: string, feeling: string) => void; // Callback for mood changes
...
const AIResponseBubble: React.FC<AIResponseBubbleProps> = ({ 
  ...
  // Load the first emotion selection from database on mount (if no emotion provided)
  useEffect(() => {
  ...
  // Update indices when emotion/feeling props change
  useEffect(() => {
    if (emotion) {
