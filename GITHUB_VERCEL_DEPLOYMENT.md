...
- ✅ `vercel.json` file in project root (already configured)
...
2. **Create a GitHub Release**
   ...
   - **Tag version**: `v1.12.25` (must match `package.json` version)
   - **Release title**: `Grounded PWA v1.12.25`
...
git tag -a v1.12.25 -m "Release v1.12.25"
git push origin v1.12.25
...
1. **GitHub Push/Release** → Triggers Vercel webhook
2. **Vercel Build** → Runs `npm install` and `npm run build`
3. **Vercel Deploy** → Deploys `dist/` folder to CDN
4. **Vercel Config** → Applies headers from `vercel.json`
5. **Live URL** → App available at `your-project.vercel.app`
...
- [ ] **Headers are correct** (check DevTools → Network)
...
**Quick Start**: Push to `main` branch → Vercel auto-deploys → Done! 🚀
